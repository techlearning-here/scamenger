...
1. **Production:** Create a project at [supabase.com](https://supabase.com) (e.g. **scamenger-prod**). Run `supabase/migrations/001_full_schema.sql` in **SQL Editor**. Use this project’s URL and keys for Render and Vercel (steps 2–3 below).
2. **Development (optional):** Create a second project (e.g. **scamenger-dev**), run `001_full_schema.sql` there too, and use its URL and service_role key in `backend/.env` for local development.
...
- **Contact form:** The public contact form (`/contact`) submits messages to the backend; they are stored in the `contact_messages` table (created by `001_full_schema.sql`). Admins can view, read, and delete messages at **Admin → View messages** (`/z7k2m9/messages/`).
...
2. **Supabase migration**  
   The `reports` table must exist. In Supabase → **SQL Editor**, run `supabase/migrations/001_full_schema.sql` for the same project whose URL and key you use on Render.
...
2. **Supabase migration**  
   The `reports` (and related) tables must exist. In Supabase → **SQL Editor**, run **`supabase/migrations/001_full_schema.sql`** for the same project whose URL and key you use on Render. This single migration creates all tables including Facebook post tracking on reports.
