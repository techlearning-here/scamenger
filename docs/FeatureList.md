...
| 27e | **Share report to X.com** | On the public report page, let users share the report to **X (Twitter)** with the same flow as Facebook/WhatsApp: show formatted content in a modal, allow editing, then open X share intent. *(Implement later.)* | Planned |
| 27f | **Share report to Threads** | On the public report page, let users share the report to **Threads** with the same flow as Facebook/WhatsApp: show formatted content in a modal, allow editing, then open Threads share intent. *(Implement later.)* | Planned |
...
Stories index at `/stories/` and per-story pages at `/stories/[slug]`. Real, anonymized scam stories; category filter and themed cards.

| # | Feature | Description | Status |
|---|---------|-------------|--------|
| 17j | **Stories index page** | Hero, intro, category filter (pills + select), grid of story cards with icon + category + title; each card links to `/stories/[slug]`. Category-themed card colours and accents. | Done |
