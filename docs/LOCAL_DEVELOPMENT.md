...
For **admin login** (e.g. `/z7k2m9/`): set `ADMIN_USERNAME`, `ADMIN_PASSWORD`, and `ADMIN_SECRET` if you want to override the defaults (see `backend/.env.example`). The app ships with dev defaults so admin can work without setting these.
...
- Admin dashboard (after setting admin credentials or using defaults): **http://localhost:3000/z7k2m9/**.
...
- **Admin login fails:** Set `ADMIN_USERNAME`, `ADMIN_PASSWORD`, and `ADMIN_SECRET` in `backend/.env` (or rely on the built-in dev defaults; see `backend/app/core/config.py`).
